...
class ObjectMonitor {
 ...
 public:
  ...
  ObjectMonitor() {
    ...
    _recursions   = 0; //重入次数 
    _object       = NULL;
    _owner        = NULL; //当前monitor的线程 
    _WaitSet      = NULL; //处于wait状态的线程set 
    _WaitSetLock  = 0 ;
    _Responsible  = NULL ;
    _succ         = NULL ;
    _cxq          = NULL ; //jvm为每个尝试进入synchornized代码段的javaThread创建一个ObjectWaiter并添加到_cxq队列中 
    FreeNext      = NULL ;
    _EntryList    = NULL ; // 处于等待锁block状态的线程，由ObjectWaiter组成的双向链表，JVM会从该链表中去除一个ObjectWaiter并唤醒对应的javaThread 
    _SpinFreq     = 0 ;
    _SpinClock    = 0 ;
    OwnerIsThread = 0 ;
    _previous_owner_tid = 0; //监视器前一个持有者的线程id 
  }
